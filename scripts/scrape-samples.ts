...
const INSTRUMENTS = [
	{
		...
		urlPattern: /href="(sound files\/[^"]*\.ff\.([a-zA-Z0-9]+)\.aiff)"/g
	}
	// {
	//     name: 'guitar',
...
const main = async (instrument: (typeof INSTRUMENTS)[number]) => {
	...
	const files = (() => {
		const matches = text.matchAll(instrument.urlPattern);

		return Array.from(matches).map((m) => ({
			url: new URL(m[1], instrument.url).toString(),
			note: m[2]
		}));
	})();
	...
	for (const { url, note } of files) {
		...
		const tempAiffPath = createTempFile('.aiff');
		await downloadAudio(url, tempAiffPath);

		const mp3Path = path.resolve(SAMPLES_DIR, instrument.name, `${note}.mp3`);
		await execPromise(
			`ffmpeg -i "${tempAiffPath}" -af "silenceremove=1:0:-40dB" -q:a 0 "${mp3Path}"`
		);
	}
};
